export type Lang = "en" | "mr";

export const translations = {
  en: {
    topbar: {
      welcome: "Welcome to the Official Portal of Chhatrapati Sambhajinagar Municipal Corporation",
      skip: "Skip to main content",
      skipNav: "Go to Main Content",
      skipDir: "Go to Navigation",
      mobileApp: "My CSMC Mobile App",
      screen: "Screen Reader",
      sitemap: "Sitemap",
      help: "Help",
      employeeLogin: "Employee Login",
      userPolicy: "User Manual",
      citizenLogin: "Citizen Login",
    },
    nav: {
      home: "Home",
      about: "About",
      departments: "Departments",
      services: "Citizen Services",
      tenders: "Tenders",
      notices: "Notices",
      contact: "Contact",
      publications: "Publications",
      grievance: "Grievance",
      disasterMgmt: "Disaster Management",
      recruitment: "Recruitment",
      electionVoter: "Voter List",
      faq: "FAQ",
      rtiAct: "RTI Act",
      govtOrders: "Govt. Orders",
      mahanagarpalika: "Mahanagarpalika",
      mahanagarpalikaItems: [
        { label: "About CSMC & Chhatrapati Sambhajinagar", to: "/about" },
        { label: "Hon'ble Municipal Commissioner", to: "/commissioner" },
        { label: "Departments", to: "/departments" },
        { label: "Hon'ble Mayors' List", to: "/about#mayors" },
        { label: "Hon'ble Deputy Mayors' List", to: "/deputy-mayors-list" },
        { label: "Hon'ble Commissioners' List", to: "/about#commissioners" },
        { label: "Minutes of Standing Committee", to: "/about#minutes" },
      ],
    },
    org: {
      name: "Chhatrapati Sambhajinagar Municipal Corporation",
      short: "CSMC",
      tagline: "Tourism Capital of Maharashtra · City of Heritage, Vision of Tomorrow",
    },
    hero: {
      eyebrow: "",
      title: "Building Cleaner, Greener, Smarter\nChhatrapati Sambhajinagar",
      subtitle: "",
      ctaPrimary: "Pay Property Tax",
      ctaSecondary: "File a Complaint",
      tabHome: "Home",
      tabNotices: "Notices",
    },
    quick: {
      title: "Quick Services",
      subtitle: "Most-used citizen services, just one click away.",
      items: [
        { t: "Pay Property Tax", d: "Check & Pay online", url: "https://chhsambhajinagarmc.org/TaxCollection/pg/property/getPropertyPgWebApi" },
        { t: "Pay Water Tax", d: "Check & Pay online", url: "https://chhsambhajinagarmc.org/Watersupply/pg/ledger/getWaterPgApi.do" },
        { t: "Birth Certificate", d: "Apply or download", url: "https://rts.chhsambhajinagarmc.org/links/birth-certificate" },
        { t: "Death Certificate", d: "Apply or download", url: "https://rts.chhsambhajinagarmc.org/links/death-certificate" },
        { t: "Trade License", d: "New Trade License", url: "https://aurangabadmahapalika.org/RTSservices/ws/rtiapplication/newRTITradeLicenceNew.do" },
        { t: "Building Permission", d: "Plan approval status", url: "https://rts.chhsambhajinagarmc.org/links/dashboard" },
        { t: "Grievance", d: "Lodge & track complaints", url: "https://aurangabadmahapalika.org/csms/complaint_form.php" },
        { t: "Gunthewari Challan", d: "Gunthewari regularisation", url: "https://rts.chhsambhajinagarmc.org/links/gunthewari_form_codev2" },
        { t: "Ramai Awas Yojana", d: "Housing scheme login portal", url: "https://chhsambhajinagarmc.org/RAMAI/ws/user/login.do" },
        { t: "All RTS Services", d: "View all RTS citizen services", url: "https://rts.chhsambhajinagarmc.org/links/dashboard" },
      ],
    },
    mayor: {
      tag: "Message from the Commissioner",
      title: "Serving the citizens with transparency and care",
      body:
        "Chhatrapati Sambhajinagar is a city where centuries of heritage meet modern aspiration. Our administration is committed to delivering responsive, technology-driven civic services that make daily life simpler for every resident. Together, we are shaping a model city — clean, inclusive and ready for tomorrow.",
      name: "Shri Amol Yedge, IAS",
      role: "Municipal Commissioner & Administrator",
    },
    commissioner: {
      name: "Shri Amol Yedage, IAS",
      nameMarathi: "श्री. अमोल येडगे, भा.प्र.से.",
    },
    notices: { title: "Latest Notices & Announcements", view: "View all" },
    announcements: {
      label: "What's New",
      items: [
        "Gunthewari Challan Calculator",
        "Live City Alerts & Public Advisory",
        "Property tax 10% rebate notice",
        "SWM Phase II e-tender live",
        "Latest municipal notices",
      ],
    },
    stats: { title: "Our City in Numbers" },
    statsItems: [
      { v: "17.5L+", l: "Population Served" },
      { v: "10", l: "Administrative Zones" },
      { v: "116", l: "Wards" },
      { v: "29", l: "Prabhags" },
      { v: "138", l: "Gardens & Parks" },
      { v: "₹3,000 Cr", l: "Annual Budget" },
    ],
    explore: { title: "Explore Sambhajinagar", subtitle: "Heritage, culture and modern landmarks." },
    cta: { title: "Need help with a civic service?", body: "Our 24×7 helpline and online grievance portal are always at your service.", btn: "Lodge a Grievance" },
    footer: {
      about: "Official portal of the Chhatrapati Sambhajinagar Municipal Corporation, governing the historic city formerly known as Aurangabad.",
      quick: "Quick Links",
      services: "Citizen Services",
      contact: "Contact Us",
      address: "Chhatrapati Sambhajinagar Municipal Corporation, Main Building, Town Hall, behind Head Post Office, Chhatrapati SambhajiNagar, Maharashtra, India, 431001",
      phone: "",
      email: "",
      rights: "© 2026 Chhatrapati Sambhajinagar Municipal Corporation. All rights reserved.",
    },
    facilities: {
      title: "Public Facilities",
      allTitle: "All Public Facilities",
      eyebrow: "Citizen Services",
      hubSubtitle: "Discover all municipal services and infrastructure across Chhatrapati Sambhajinagar.",
      exploreTitle: "Explore Public Facilities",
      exploreAll: "Explore all public facilities",
      viewAll: "View All",
      back: "Back to Public Facilities",
      searchPlaceholder: "Filter by name, address, zone, or details…",
      filterSr: "Filter facilities",
      clearSelection: "Clear selection",
      facilityOne: "facility",
      facilityMany: "facilities",
      of: "of",
      loading: "Loading facilities...",
      loadError: "Unable to load facility list.",
      emptyCategory: "Facility records for this category are not yet available.",
      noMatch: "No facilities match this filter.",
      notFound: "Category not found",
      notFoundHint: "Please return to the Public Facilities page.",
      selectAria: "Select",
      selectedFacility: "Selected facility",
      selectedFacilityAria: "Selected facility details",
      zoneWard: "Zone / ward",
      contact: "Contact",
      timings: "Timings",
      openInMaps: "Open in Maps",
      searchInMaps: "Search in Maps",
      directions: "Directions / Open in Maps",
      searchLocation: "Search location in Maps",
      mapUnavailable: "Map coordinates are not available for these facilities.",
      breadcrumbAria: "Breadcrumb",
      zonePrefix: "Zone",
      wardPrefix: "Ward",
      zoneOffice: "Zone Office",
      prevFacilities: "Previous facilities",
      nextFacilities: "Next facilities",
    },
  },
  mr: {
    topbar: {
      welcome: "छत्रपती संभाजीनगर महानगरपालिकेच्या अधिकृत संकेतस्थळावर आपले स्वागत आहे",
      skip: "मुख्य मजकुरावर जा",
      skipNav: "मुख्य विषयाकडे जा",
      skipDir: "दिशादर्शनाकडे जा",
      mobileApp: "माय सीएसएमसी मोबाईल ॲप",
      screen: "स्क्रीन रीडर",
      sitemap: "साइटमॅप",
      help: "मदत",
      employeeLogin: "कर्मचारी लॉग-इन",
      userPolicy: "वापरकर्ता नियमावली",
      citizenLogin: "नागरिक लॉगिन",
    },
    nav: {
      home: "मुख्यपृष्ठ",
      about: "आमच्याबद्दल",
      departments: "विभाग",
      services: "नागरिक सेवा",
      tenders: "निविदा",
      notices: "जाहीर सूचना",
      contact: "संपर्क",
      publications: "प्रकाशने",
      grievance: "अभिप्राय",
      disasterMgmt: "आपत्ती व्यवस्थापन",
      recruitment: "भरती पृष्ठ",
      electionVoter: "निवडणूक",
      faq: "सामान्य प्रश्न",
      rtiAct: "लोकसेवा हक्क अधिनियम",
      govtOrders: "शासन निर्णय / परिपत्रके",
      mahanagarpalika: "महानगरपालिका",
      mahanagarpalikaItems: [
        { label: "CSMC व छत्रपती संभाजीनगर बद्दल", to: "/about" },
        { label: "विभाग", to: "/departments" },
        { label: "मा. महापौरांची यादी", to: "/about#mayors" },
        { label: "मा. उपमहापौरांची यादी", to: "/deputy-mayors-list" },
        { label: "मा. आयुक्तांची यादी", to: "/about#commissioners" },
        { label: "स्थायी समिती इतिवृत्त", to: "/about#minutes" },
      ],
    },
    org: {
      name: "छत्रपती संभाजीनगर महानगरपालिका",
      short: "छ.सं.म.न.पा.",
      tagline: "महाराष्ट्राची पर्यटन राजधानी · शहर वारसाचे, स्वप्न उद्याचे",
    },
    hero: {
      eyebrow: "",
      title: "स्वच्छ, हरित आणि स्मार्ट\nछत्रपती संभाजीनगर घडवू\u00A0या",
      subtitle: "",
      ctaPrimary: "मालमत्ता कर भरा",
      ctaSecondary: "तक्रार नोंदवा",
      tabHome: "मुख्यपृष्ठ",
      tabNotices: "सूचना",
    },
    quick: {
      title: "जलद सेवा",
      subtitle: "नागरिकांसाठी सर्वाधिक वापरल्या जाणार्‍या सेवा एका क्लिकवर.",
      items: [
        { t: "मालमत्ता कर भरा", d: "पहा व ऑनलाइन भरा", url: "https://chhsambhajinagarmc.org/TaxCollection/pg/property/getPropertyPgWebApi" },
        { t: "पाणी कर भरा", d: "पहा व ऑनलाइन भरा", url: "https://chhsambhajinagarmc.org/Watersupply/pg/ledger/getWaterPgApi.do" },
        { t: "जन्म प्रमाणपत्र", d: "अर्ज करा / डाउनलोड करा", url: "https://rts.chhsambhajinagarmc.org/links/birth-certificate" },
        { t: "मृत्यू प्रमाणपत्र", d: "अर्ज करा / डाउनलोड करा", url: "https://rts.chhsambhajinagarmc.org/links/death-certificate" },
        { t: "व्यापार परवाना", d: "नवीन व्यापार परवाना", url: "https://aurangabadmahapalika.org/RTSservices/ws/rtiapplication/newRTITradeLicenceNew.do" },
        { t: "बांधकाम परवानगी", d: "नकाशा मंजुरी स्थिती", url: "https://rts.chhsambhajinagarmc.org/links/dashboard" },
        { t: "तक्रार", d: "नोंदवा व स्थिती तपासा", url: "https://aurangabadmahapalika.org/csms/complaint_form.php" },
        { t: "गुंठेवारी चलन", d: "गुंठेवारी नियमितीकरण", url: "https://rts.chhsambhajinagarmc.org/links/gunthewari_form_codev2" },
        { t: "रमाई आवास योजना", d: "गृहनिर्माण योजना लॉगिन पोर्टल", url: "https://chhsambhajinagarmc.org/RAMAI/ws/user/login.do" },
        { t: "सर्व RTS सेवा", d: "सर्व RTS नागरी सेवा पहा", url: "https://rts.chhsambhajinagarmc.org/links/dashboard" },
      ],
    },
    mayor: {
      tag: "आयुक्तांचा संदेश",
      title: "पारदर्शकतेने व आपुलकीने नागरिकांची सेवा",
      body:
        "छत्रपती संभाजीनगर हे शतकानुशतकांचा वारसा आणि आधुनिक आकांक्षा यांचा संगम असलेले शहर आहे. तंत्रज्ञानाधारित, प्रतिसादात्मक नागरी सेवा देण्यासाठी आमचे प्रशासन कटिबद्ध आहे — स्वच्छ, समावेशक आणि भविष्यासाठी सज्ज शहर घडवण्याच्या दिशेने आम्ही एकत्रित वाटचाल करत आहोत.",
      name: "श्री. अमोल येडगे, भा.प्र.से.",
      role: "महानगरपालिका आयुक्त व प्रशासक",
    },
    commissioner: {
      name: "Shri Amol Yedage, IAS",
      nameMarathi: "श्री. अमोल येडगे, भा.प्र.से.",
    },
    notices: { title: "ताज्या सूचना व घोषणा", view: "सर्व पहा" },
    announcements: {
      label: "नवीन काय",
      items: [
        "गुंठेवारी चलन कॅल्क्युलेटर",
        "थेट शहर इशारे व सार्वजनिक सल्ला",
        "मालमत्ता कर १०% सवलत सूचना",
        "घनकचरा प्रकल्प टप्पा-२ ई-निविदा",
        "ताज्या महापालिका सूचना",
      ],
    },
    stats: { title: "आपले शहर — आकड्यांत" },
    statsItems: [
      { v: "१७.५ लाख+", l: "सेवा देत असलेली लोकसंख्या" },
      { v: "१०", l: "प्रशासकीय विभाग" },
      { v: "११६", l: "वॉर्ड्स" },
      { v: "२९", l: "प्रभाग समूह" },
      { v: "१३८", l: "बागा व उद्याने" },
      { v: "₹३,००० कोटी", l: "वार्षिक अर्थसंकल्प" },
    ],
    explore: { title: "संभाजीनगर अनुभवा", subtitle: "वारसा, संस्कृती आणि आधुनिक खुणा." },
    cta: { title: "नागरी सेवेसाठी मदत हवी?", body: "आमची २४×७ हेल्पलाइन व ऑनलाइन तक्रार पोर्टल आपल्या सेवेसाठी सदैव उपलब्ध.", btn: "तक्रार नोंदवा" },
    footer: {
      about: "छत्रपती संभाजीनगर महानगरपालिकेचे अधिकृत संकेतस्थळ — पूर्वीचे औरंगाबाद.",
      quick: "त्वरित दुवे",
      services: "नागरिक सेवा",
      contact: "संपर्क",
      address: "छत्रपती संभाजीनगर महानगरपालिका, मुख्य इमारत, टाऊन हॉल, हेड पोस्ट ऑफिसच्या मागे, छत्रपती संभाजीनगर, महाराष्ट्र, भारत, ४३१००१",
      phone: "",
      email: "",
      rights: "© २०२६  छत्रपती संभाजीनगर महानगरपालिका. सर्व हक्क राखीव.",
    },
    facilities: {
      title: "सार्वजनिक सुविधा",
      allTitle: "सर्व सार्वजनिक सुविधा",
      eyebrow: "नागरिक सेवा",
      hubSubtitle: "छत्रपती संभाजीनगरमधील सर्व महापालिका सेवा व पायाभूत सुविधा शोधा.",
      exploreTitle: "सार्वजनिक सुविधांचा शोध घ्या",
      exploreAll: "सर्व सार्वजनिक सुविधा पहा",
      viewAll: "सर्व पहा",
      back: "सार्वजनिक सुविधांकडे परत जा",
      searchPlaceholder: "नाव, पत्ता, झोन किंवा तपशीलाने शोधा…",
      filterSr: "सुविधा गाळा",
      clearSelection: "निवड साफ करा",
      facilityOne: "सुविधा",
      facilityMany: "सुविधा",
      of: "/",
      loading: "सुविधा लोड करत आहे...",
      loadError: "सुविधा यादी लोड करणे शक्य नाही.",
      emptyCategory: "या वर्गासाठी सुविधा नोंदी अद्याप उपलब्ध नाहीत.",
      noMatch: "या गाळण्याला जुळणाऱ्या सुविधा नाहीत.",
      notFound: "वर्ग आढळला नाही",
      notFoundHint: "कृपया सार्वजनिक सुविधा पृष्ठावर परत या.",
      selectAria: "निवडा",
      selectedFacility: "निवडलेली सुविधा",
      selectedFacilityAria: "निवडलेल्या सुविधेचा तपशील",
      zoneWard: "झोन / प्रभाग",
      contact: "संपर्क",
      timings: "वेळा",
      openInMaps: "नकाशात पहा",
      searchInMaps: "नकाशावर शोधा",
      directions: "दिशा / नकाशात उघडा",
      searchLocation: "नकाशात स्थान शोधा",
      mapUnavailable: "या सुविधांसाठी नकाशा निर्देशांक उपलब्ध नाहीत.",
      breadcrumbAria: "ब्रेडक्रंब",
      zonePrefix: "झोन",
      wardPrefix: "प्रभाग",
      zoneOffice: "झोन कार्यालय",
      prevFacilities: "मागील सुविधा",
      nextFacilities: "पुढील सुविधा",
    },
  },
};

export type Translations = (typeof translations)["en"];